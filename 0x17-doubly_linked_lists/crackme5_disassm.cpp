...
uint32_t f4(struct s0* rdi, uint32_t esi) {
    ...
    v4 = esi; // arg_1 len
    eax5 = v3->f0; // arg_1[0]
    v6 = reinterpret_cast<uint32_t>(static_cast<int32_t>(*reinterpret_cast<signed char*>(&eax5))); // arg_1[0] as uint32, will be the maximum char
    v7 = 0;
	// iterates through arg_1
    while (reinterpret_cast<int32_t>(v7) < reinterpret_cast<int32_t>(v4)) {
        eax8 = *reinterpret_cast<unsigned char*>(reinterpret_cast<int64_t>(v3) + v7); // current char
		// if (int32)eax8 > v6 (arg_1[0])
        if (static_cast<int32_t>(*reinterpret_cast<signed char*>(&eax8)) > reinterpret_cast<int32_t>(v6)) {
            eax9 = *reinterpret_cast<unsigned char*>(reinterpret_cast<int64_t>(v3) + v7); // current char
            v6 = reinterpret_cast<uint32_t>(static_cast<int32_t>(*reinterpret_cast<signed char*>(&eax9))); // set v6 to current char
        }
        ++v7;
    }
    *reinterpret_cast<uint32_t*>(&rdi10) = v6 ^ 14;
    *reinterpret_cast<int32_t*>(reinterpret_canst<int64_t>(&rdi10) + 4) = 0;
    fun_400570(rdi10);
...
uint32_t f5(struct s0* rdi, uint32_t esi) {
	...
	// iterates through arg_1
    while (reinterpret_cast<int32_t>(v6) < reinterpret_cast<int32_t>(v4)) {
        eax7 = *reinterpret_cast<unsigned char*>(reinterpret_cast<int64_t>(v3) + v6); // current char
        eax8 = *reinterpret_cast<unsigned char*>(reinterpret_cast<int64_t>(v3) + v6); // current char

		// v5 is the sum of the square of each char
...
uint32_t f6(int64_t rdi, int64_t rsi) {
    ...
    eax3 = *reinterpret_cast<int32_t*>(&rdi); // arg_1[0]
    v4 = *reinterpret_cast<signed char*>(&eax3); // arg_1[0]
    v5 = 0;
    v6 = 0;
...
void segf() {
    ...
    rand = -1;
    fun_400580();
    rbp1 = reinterpret_cast<void*>(reinterpret_cast<int64_t>(__zero_stack_offset()) - 8 - 16 - 8 + 8 - 8); // 0x7ffffffee270
    rax2 = g28;
    if (!0) {
        segf();
    }
    v3 = rsi4->f8;// arg_1
    v5 = rsi4->f16;// arg_2
    rax6 = fun_400540(v5);//strlen(arg_2)
    if (rax6 != 6) {
        segf();
    }
    rax7 = fun_400540(v3);// strlen(arg_1)
	// since the result involves &63, it can only range from 0 to 63
    eax8 = f1(*reinterpret_cast<uint32_t*>(&rax7));// (x ^ 59)&63 / /3:56 | 4:63
    edx9 = v5->f0; // arg_2[0]
	// since eax must be between 0 and 63, eax10 must be between rbp1 - 80 and rbp1 - 17
    ...
    eax11 = f2(v3, *reinterpret_cast<uint32_t*>(&rax7));// (sumof(arg_1) ^ 79) & 63
    edx12 = v5->f1;//arg_2[1]
    ...
    eax14 = f3(v3, *reinterpret_cast<uint32_t*>(&rax7)); // (product_of(arg_1) ^ 85) & 63
    edx15 = v5->f2;
    ...
    eax17 = f4(v3, *reinterpret_cast<uint32_t*>(&rax7)); // uses the ((max char of arg_1) ^ 14) as seed for srand, then returns the (rand() & 63)
    edx18 = v5->f3;
    ...
    *reinterpret_cast<uint32_t*>(&rsi20) = *reinterpret_cast<uint32_t*>(&rax7);// arg_1 len
    *reinterpret_cast<int32_t*>(reinterpret_cast<int64_t>(&rsi20) + 4) = 0;
    eax21 = f5(v3, *reinterpret_cast<uint32_t*>(&rsi20)); // ((sum of the square of arg_1) ^ 0xef) & 63
    edx22 = v5->f4;
...
uint32_t f2(struct s0* rdi, uint32_t esi) {
	...
	// iterates through arg_1
    while (reinterpret_cast<int32_t>(v6) < reinterpret_cast<int32_t>(v4)) {
		// eax5 is the current char
        eax7 = *reinterpret_cast<unsigned char*>(reinterpret_cast<int64_t>(v3) + v6);
		// v5 should be the sum of all chars (they should be signed)
